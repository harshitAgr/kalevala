...
def _seed(tmp_log_repo: Path, date_str: str, sid: str, body_extra: str = "") -> None:
    ...
    s = SessionSummary(
        ...
        open_threads=[], time_range=["09:00", "09:30"], project="opet",
...
def test_search_finds_matches(tmp_config: Path, tmp_log_repo: Path):
    _seed(tmp_log_repo, "2026-04-17", "sid-1", body_extra="fixed val loop crash")
    _seed(tmp_log_repo, "2026-04-16", "sid-2", body_extra="totally unrelated")
    proc = _run(tmp_config, "search", "val loop")
    assert proc.returncode == 0
...
def test_resume_prints_command(tmp_config: Path, tmp_log_repo: Path):
    _seed(tmp_log_repo, "2026-04-17", "target-id", body_extra="the val loop fix")
    proc = _run(tmp_config, "resume", "val loop fix")
    assert proc.returncode == 0
